...
source kernel_platform/build/kernel/oplus/oplus_setup.sh $1 $2
...
rebuild_vendor_boot_image() {
    ...
    do
        ...
        ${LZ4} -d ${VENDOR_BOOT_TMP_IMAGE}/vendor_ramdisk${index}.lz4 ${VENDOR_BOOT_TMP_IMAGE}/vendor_ramdisk${index}
        ...
        ${LZ4} -l -12 --favor-decSpeed ${VENDOR_BOOT_TMP_IMAGE}/vendor_ramdisk${index} ${VENDOR_BOOT_TMP_IMAGE}/vendor_ramdisk${index}.lz4
...
rebuild_dtbo_image() {
    ...
    if [ -d ${KERNEL_OUT}/dtbs ];then
        prj=($(find ${KERNEL_OUT}/dtbs -type f -name *${project}*-dtbo.img -print))
        echo ${prj[@]}
        if [ ! -z "$prj" ]; then
            for e in ${prj[@]}
            do
                dtbo_name=`basename $e`
                echo "cp ${KERNEL_OUT}/dtbs/${dtbo_name}  ${IMAGE_OUT}/dtbo.img"
                if [ -f ${KERNEL_OUT}/dtbs/${dtbo_name} ]; then
                    cp ${KERNEL_OUT}/dtbs/${dtbo_name}  ${IMAGE_OUT}/dtbo.img
                fi
            done
        else
            #if not support different project dtbo.img copy default dtbo.img
            cp ${KERNEL_OUT}/dtbs/dtbo.img  ${IMAGE_OUT}/dtbo.img
        fi
    fi
